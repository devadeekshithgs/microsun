...
type WorkerRegisterFormValues = z.infer<typeof workerRegisterSchema>;
...
export default function WorkerRegister() {
    ...
    const form = useForm<WorkerRegisterFormValues>({
    ...
    const onSubmit = async (values: WorkerRegisterFormValues) => {
        ...
        try {
            const { error } = await supabase.auth.signUp({
                ...
                options: {
                    emailRedirectTo: `${window.location.origin}/worker-register`,
                    data: {
                        full_name: values.fullName,
                        phone: values.phone,
                        registration_type: 'worker', // This will be used by the trigger
                    },
                },
            });
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            const { error } = await supabase.auth.signInWithOAuth({
                provider: 'google',
                options: {
                    redirectTo: `${window.location.origin}/worker-register`,
                    queryParams: {
                        access_type: 'offline',
                        prompt: 'consent',
                    },
            ...
            <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
                <Card className="w-full max-w-md text-center">
                    <CardHeader className="space-y-4">
                        ...
                        <CardTitle className="text-2xl">Worker Registration Successful!</CardTitle>
                        <CardDescription className="text-base">
                            Your worker account has been created. You can now log in to access the worker portal.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            <Card className="w-full max-w-md">
                <CardHeader className="space-y-4 text-center">
                    ...
                    <div>
                        ...
                        <CardDescription>Register as a MicroSun worker</CardDescription>
                    </div>
                </CardHeader>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input type="email" placeholder="you@example.com" autoComplete="email" className="h-12" {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
